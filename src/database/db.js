...
export const getBooksStore = async (id) => {
    if (!id) {
        const storeBooksDate = await prisma.store_lib.findMany({}).then(res => res).then(res => res).catch(err => console.log('Error', err))
    ...
    const storeBooksDate = await prisma.store_lib.findUniqueOrThrow({
        where: {id: id}
    }).then(res => res).catch(err => console.log('Error', err))
